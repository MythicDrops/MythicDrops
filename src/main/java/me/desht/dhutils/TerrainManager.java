...
public final class TerrainManager {
   ...
   */
  public void saveTerrain(File saveFile, Location l1, Location l2)
      throws FilenameException, DataException, IOException {
    ...
    CuboidClipboard
        clipboard =
        new CuboidClipboard(max.subtract(min).add(new Vector(1, 1, 1)), min);
  ...
  /**
   * Load the data from the given schematic file and paste it at the given location.  If the
   * location is null, then paste it at the saved data's origin.
   *
  ...
  public void loadSchematic(File saveFile, Location loc)
      throws FilenameException, DataException, IOException, MaxChangedBlocksException,
